...
public class SimulationServiceImpl implements SimulationService{
    ...
    SimulationServiceImpl(ParametersService parametersService,PackageService packageService,
                          TruckService truckService){
...
//                }
    ...
    public void run(MultipartFile file1, MultipartFile file2, SseEmitter sseEmitter, String guid) {
        try {
            ...
            /**Load Trucks**/
            ArrayList<Truck> trucks = truckService.getAllTrucks();
            for(Truck truck: trucks){
                truck.reinitialize();
            }
            ...
            Date initialtime = new Date(orders.get(0).getDateOrder().getTime());
            ...
            Date endTime = new Date(initialtime.getTime() + 72*HOUR );
            ...
            int counter = 1;
            Date secondTruck = new Date();
            Date fourthTruck = new Date();
            Truck truck2 = null;
            Truck truck4 = null;
            ...
            while(timer.getTime()<= endTime.getTime()){
                ...
                if(timer.getTime() == secondTruck.getTime()){
                    System.out.println("Segundo vehiculo averiado : " + truck2.getId());
                    System.out.println("X: " + truck2.getX() + ", Y: " + truck2.getY());
                    truck2.setAveriado(true);
                    while(truck2.getPackages().size() > 0){
                        Package pack = truck2.getPackages().remove(0);
                        if(pack.getIsOrder()){
                            processPackages.add(pack);
                        }
                    }
                    truck2.setRoute(new ArrayList<>());
                }
                if(timer.getTime() == fourthTruck.getTime()){
                    System.out.println("Cuarto vehiculo averiado : " + truck4.getId());
                    System.out.println("X: " + truck4.getX() + ", Y: " + truck4.getY());
                    truck4.setAveriado(true);
                    while(truck4.getPackages().size() > 0){
                        Package pack = truck4.getPackages().remove(0);
                        if(pack.getIsOrder()){
                            processPackages.add(pack);
                        }
                    }
                    truck4.setRoute(new ArrayList<>());
                }
                if(timer.getTime() == (secondTruck.getTime() + 1*3600*1000)){
                    System.out.println("Segundo vehiculo mantenimiento : " + truck2.getId());
                    truck2.setAveriado(false);
                    truck2.setActive(false);
                }
                if(timer.getTime() == (fourthTruck.getTime() + 1*3600*1000)){
                    System.out.println("Cuarto vehiculo mantenimiento : " + truck4.getId());
                    truck4.setAveriado(false);
                    truck4.setActive(false);
                }
                if(timer.getTime() == (secondTruck.getTime() + 48*3600*1000)){
                    System.out.println("Segundo vehiculo operativo");
                ...
                if(timer.getTime() == (fourthTruck.getTime() + 48*3600*1000)){
                    System.out.println("Cuarto vehiculo operativo");
                ...
                if(timer.getMinutes() % BATCH_TIME == 0){
                    ...
                    /** Se revisan los paquetes en deuda **/
                    ...
                    /** Se filtran los camiones activos **/

                    ArrayList<Truck> trucksOperativos = new ArrayList<>();

                    for(Truck truck: trucks){
                        if(truck.getActive() && !truck.isAveriado()){
                            trucksOperativos.add(truck);
                        }
                    }
                    VRPTW problem = new VRPTW(trucksOperativos,processPackages,map,timer);
                    problem.solve();
                    ...
                    for(Truck truck: trucks){
                        if(counter <= 4 && truck.getPackages().size() > 1){
                            if(truck.getCounter() == 0){
                                truck.setCounter(counter);
                                if(counter == 2){
                                    secondTruck = new Date(timer.getTime() + 2*3600*1000);
                                    truck2 = truck;
                                }
                                if(counter == 4){
                                    fourthTruck = new Date(timer.getTime() + 3*3600*1000);
                                    truck4 = truck;
                                }
                                counter++;
                            }
                    ...
                    Collections.sort(trucks,(o1, o2) -> o1.getId() - o2.getId());
                    payload.put("indicator",getIndicator());
